...
/**
 * Fetch + parse an index once per URL. Returns a cached promise resolving to
 * the `data` array (empty on any failure — a single warning is logged).
 * @param {string} url
  ...
  if (!indexCache.has(url)) {
    const promise = fetch(url)
      ...
      .catch((e) => {
        ...
        console.warn(`cards-teaser: could not load index ${url} —`, e.message);
        return [];
      });
