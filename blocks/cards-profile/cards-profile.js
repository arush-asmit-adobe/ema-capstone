 ...
 * the sheet (no code, no page) and the team grid updates. Path is resolved per
 * environment (local preview serves content under a /content prefix).
 */
const TEAM_SHEET = '/us/en/data/team.json';
...
/**
 * Turn each social <a> into a compact icon strip beneath the name and role.
 ...
 *
 * The team grid renders dynamically from an author-editable data sheet
 * (/us/en/data/team.json): one row per person, with name, role, image, and
 * facebook/twitter/instagram links. Authors add or remove a row in the sheet
 * and the grid updates — no code and no page needed. If the sheet is
 * unavailable, falls back to any authored block-table content.
 *
 * @param {Element} block The block element
 */
export default async function decorate(block) {
  try {
    const resp = await fetch(resolveSheet(TEAM_SHEET), { cache: 'no-cache' });
    if (!resp.ok) throw new Error(`team sheet ${resp.status}`);
    const json = await resp.json();
    const records = json.data || [];
    if (!records.length) throw new Error('no team records');
