...
function buildCard(record) {
  ...
  // The single title link below is "stretched" over the whole card (see CSS),
  // so one focusable link covers the entire card surface (image included) with
  // a single tab stop — the image is not wrapped in its own anchor.
  const imageDiv = document.createElement('div');
  imageDiv.className = 'cards-index-card-image';
  if (record.image) {
  ...
  body.className = 'cards-index-card-body';
  const h3 = document.createElement('h3');
  const titleLink = document.createElement('a');
  titleLink.href = resolveHref(record.path);
  titleLink.textContent = record.title;
  titleLink.className = 'cards-index-card-link';
  li.classList.add('cards-index-card-clickable');
  h3.append(titleLink);
  body.append(h3);
...
function decorateAuthored(block) {
  ...
  [...block.children].forEach((row) => {
    ...
    [...li.children].forEach((div) => {
      if (div.children.length === 1 && div.querySelector('picture')) div.className = 'cards-index-card-image';
      else div.className = 'cards-index-card-body';
    });
    ul.append(li);
  });
  ...
  [...ul.children].forEach((li) => {
    ...
    if (titleLink) {
      titleLink.classList.add('cards-index-card-link');
      li.classList.add('cards-index-card-clickable');
    }
  });
  block.textContent = '';
...
function decorateSectionCta(block) {
  ...
  ctas.forEach((a) => {
    ...
    button.type = 'button';
    button.className = 'cards-index-cta';
...
function buildTablist(records, ul) {
  ...
  tablist.className = 'cards-index-tabs';
  ...
  labels.forEach((label, i) => {
    ...
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'cards-index-tab';
...
export default async function decorate(block) {
  ...
  try {
    ...
    console.warn('cards-index: falling back to authored cards —', e.message);
    decorateAuthored(block);
    decorateSectionCta(block);
  }
}
